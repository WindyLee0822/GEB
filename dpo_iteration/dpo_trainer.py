...
def _tokenize(
    ...
    if model is None:
        ...
        reference_tokens = [_process_answer(prompt, [resp[idx] for resp in features['reference_response']], processor, tokenizer, images)for idx in range(len(features['reference_response'][0]))]

        prompt_len_input_ids = _adjust_prompt_length(prompt_tokens, chosen_tokens, rejected_tokens,reference_tokens)

        prompt_tokens, chosen_tokens, rejected_tokens, reference_tokens = _add_special_tokens(
            tokenizer, prompt_len_input_ids, prompt_tokens, chosen_tokens, rejected_tokens,reference_tokens
        )

        _truncate_tokens(chosen_tokens, rejected_tokens, reference_tokens,prompt_tokens, args)

        _build_sequence_tokens(batch, chosen_tokens, args, "chosen")
        _build_sequence_tokens(batch, rejected_tokens, args, "rejected")

        for idx,tokens in enumerate(reference_tokens):
            for token in tokens:
                sequence_tokens = {f"reference_{idx}_{k}": token[f"prompt_{k}"] + token[k] for k in
                                   ["input_ids", "attention_mask"]}
                sequence_tokens[f"reference_{idx}_labels"] = sequence_tokens[f"reference_{idx}_input_ids"][:]
                sequence_tokens[f"reference_{idx}_labels"][: len(token["prompt_input_ids"])] = [args.label_pad_token_id] * len(
...
def _adjust_prompt_length(
    ...
    for idx, p_tokens, c_tokens, r_tokens in zip(range(len(prompt_tokens)),prompt_tokens, chosen_tokens, rejected_tokens):
        c_len = len(c_tokens["prompt_input_ids"])
        r_len = len(r_tokens["prompt_input_ids"])
        resp_len = min([len(reference_token[idx]["prompt_input_ids"]) for reference_token in reference_tokens])
        min_len = min(c_len, r_len,resp_len)
...
def _add_special_tokens(
    ...
    rejected_tokens: List[Dict[str, List[int]]],
    reference_tokens
) -> Tuple[List[Dict[str, List[int]]], List[Dict[str, List[int]]], List[Dict[str, List[int]]], List[Dict[str, List[int]]]]:
    for i in range(len(prompt_tokens)):
        origin_prompt,origin_chosen,origin_reject = prompt_tokens[i],chosen_tokens[i],rejected_tokens[i],
        prompt_tokens[i], chosen_tokens[i], rejected_tokens[i] = add_bos_token_if_needed(
            tokenizer.bos_token_id,
            prompt_len_input_ids[i],
            origin_prompt,
            len(origin_chosen["prompt_input_ids"]),
            origin_chosen,
            len(origin_reject["prompt_input_ids"]),
            origin_reject,
        )

        for idx in range(len(reference_tokens)):
            _,_, reference_tokens[idx][i] = add_bos_token_if_needed(
                tokenizer.bos_token_id,
                prompt_len_input_ids[i],
                origin_prompt,
                len(origin_chosen["prompt_input_ids"]),
                origin_chosen,
                len(reference_tokens[idx][i]["prompt_input_ids"]),
                reference_tokens[idx][i],
            )

        for idx in range(len(reference_tokens)):
            _, reference_tokens[idx][i] = add_eos_token_if_needed(
                tokenizer.eos_token_id, chosen_tokens[i],reference_tokens[idx][i]
            )
...
def _truncate_tokens(
    chosen_tokens: List[Dict[str, List[int]]],
    rejected_tokens: List[Dict[str, List[int]]],
    reference_tokens: List[Dict[str, List[int]]],
    prompt_tokens: List[Dict[str, List[int]]],
    args: DPOConfig,
) -> None:
    """
    ...
    for idx, c_tokens, r_tokens, p_tokens in zip(range(len(chosen_tokens)),chosen_tokens, rejected_tokens, prompt_tokens):
        resp_len = max([len(resp_tokens[idx]) for resp_tokens in reference_tokens])
        longer_response_length = max(len(c_tokens["input_ids"]), len(r_tokens["input_ids"]),resp_len)
        ...
        for resp_tokens in reference_tokens:
            if len(resp_tokens[idx]["prompt_input_ids"]) + longer_response_length > args.max_length:
                if args.truncation_mode == "keep_start":
                    for k in ["prompt_input_ids", "prompt_attention_mask"]:
                        resp_tokens[idx][k] = resp_tokens[idx][k][: args.max_prompt_length]
                elif args.truncation_mode == "keep_end":
                    for k in ["prompt_input_ids", "prompt_attention_mask"]:
                        resp_tokens[idx][k] = resp_tokens[idx][k][-args.max_prompt_length :]
            ...
            if len(resp_tokens[idx]["prompt_input_ids"]) + longer_response_length > args.max_length:
                for k in ["input_ids", "attention_mask"]:
                    resp_tokens[idx][k] = resp_tokens[idx][k][: args.max_length - args.max_prompt_length]
